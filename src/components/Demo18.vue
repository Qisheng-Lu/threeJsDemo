<template>
  <div>
    <div id="three" @click="demo17"></div>
    <div class="operation-tab">
      <ul>
        <li @click="showCold">冷间参数显示</li>
      </ul>
    </div>
  </div>
</template>
...
// 创建一个矩形平面网模型，Canvas画布作为矩形网格模型的纹理贴图
      ...
      var wall = this.returnWallObject(400, 30, 1, 0, matArrayB, 0, 15, 150, "最前面的墙面（中间有个门）SnowLink");
      var door_cube1 = this.returnWallObject(20, 28, 1, 0, matArrayB, 0, 10, 150, "最前面的门SnowLink");
      var objects_cube = [];
      objects_cube.push(door_cube1);
      this.createResultBsp(wall, objects_cube,'最前面的一个门的墙SnowLink');
      this.createDoor_left(10, 24, 1, 1, 45, 12, 150, "最前面的门左门SnowLink");
      this.createDoor_right(10, 24, 1, 1, -45, 12, 150, "最前面的门右门SnowLink");
      ...
      //中间的墙
      var wall5 = this.returnWallObject(400, 30, 1, 0, matArrayB, 0, 15, 0, "中间的墙面SnowLink");
      var door_cube2 = this.returnWallObject(20, 28, 1, 0, matArrayB, -50, 10, 0, "中间的墙面左边门SnowLink");
      var door_cube3 = this.returnWallObject(20, 28, 1, 0, matArrayB, 50, 10, 0, "中间的墙面右边门SnowLink");
      var objects_cube1 = [];
      objects_cube1.push(door_cube2);
      objects_cube1.push(door_cube3);
      this.createResultBsp(wall5, objects_cube1,'中间的两个门的墙SnowLink');
      this.createDoor_left(10, 24, 1, 1, -5, 12, 0, "中间的墙面左边门左门1SnowLink");
      this.createDoor_right(10, 24, 1, 1, -95, 12, 0, "中间的墙面左边门右门1SnowLink");
      this.createDoor_left(10, 24, 1, 1, 95, 12, 0, "中间的墙面右边门左门1SnowLink");
      this.createDoor_right(10, 24, 1, 1, 5, 12, 0, "中间的墙面右边门右门1SnowLink");

      //最后面的墙
      var wall4 = this.returnWallObject(400, 30, 1, 0, matArrayB, 0, 15, -150, "最后面的墙SnowLink");
      var door_cube4 = this.returnWallObject(20, 20, 1, 0, matArrayB, -100, 16, -150, "最后面的墙左边窗户空洞SnowLink");
      var door_cube5 = this.returnWallObject(20, 20, 1, 0, matArrayB, 100, 16, -150, "最后面的墙右边窗户空洞SnowLink");
      var objects_cube3 = [];
      objects_cube3.push(door_cube4);
      objects_cube3.push(door_cube5);
      this.createResultBsp(wall4, objects_cube3,'最后面的两个窗户的墙SnowLink');
      this.createWindow(20, 20, 1, 1, -100, 16, -150, "最后面的墙左边窗户窗户1SnowLink");
      this.createWindow(20, 20, 1, 1, 100, 16, -150, "最后面的墙右边窗户窗户2SnowLink");
      ...
      gltfLoader.load( //文件加载 文件地址 我用的本地文件模拟的 vue 必须把文件放入public目录
        ...
        gltf => {
          gltf.scene.name = '这是一个破旧罐子模型SnowLink'
      ...
      this.createGrass(426, 1, 10, 0, 12.5, 0, 179.5, '草地1SnowLink')
      this.createRoad(401, 1, 25, 0, 0.5, 0, 162, '路1SnowLink')
      this.createRoad2(25, 1, 350, 0, 213, 0, -0.5, '路2SnowLink')
      ...
      this.camera.position.set(20, 200, 260); //设置相机位置
      ...
      let module = document.getElementById('three')
    ...
    methods: {
      ...
      createResultBsp(bsp,objects_cube,name){
...
// 设置纹理的旋转中心，默认(0,0)
     ...
     showCold() {
       if (!this.cold) {
         var wall1 = new THREE.BoxGeometry(60, 40, 0.01);
         var canvas = document.createElement("canvas");
         var context = canvas.getContext('2d');
         canvas.width = 120;
         canvas.height = 80;
         context.font = "12px sans-serif"
         context.fillText("面积:", 4, 14);
         context.fillText("40平米", 34, 14)
         context.fillText("库高:", 2, 30);
         context.fillText("9米", 34, 30)
         context.save()
         var texture = new THREE.CanvasTexture(canvas);
         ...
         this.board1 = new THREE.Mesh(wall1, material1); //网格模型对象Mesh
         this.board1.position.set(50, 80, -60);//设置mesh3模型对象的xyz坐标为120,0,0
         var  wall2 = new THREE.BoxGeometry(10, 10, 0.01); //创建一个立方体几何对象Geometry
         this.board2 = new THREE.Mesh(wall2, material2); //网格模型对象Mesh
         this.board2.rotateZ(Math.PI/4);//每次绕y轴旋转0.01弧度
         this.board2.position.set(50, 60, -60);//设置mesh3模型对象的xyz坐标为120,0,0
         this.scene.add(this.board1); //网格模型添加到场景中
         this.scene.add(this.board2); //网格模型添加到场景中
         this.cold = true
         this.renderA()
       }else{
         this.scene.remove(this.board1); //网格模型添加到场景中
         this.scene.remove(this.board2); //网格模型添加到场景中
         this.cold = false
         this.renderA()
       }
       },
      demo17(event){
        ...
        if (intersects.length != 0) {
          ...
          let a = this.getClickObj(selectObject)
      ...
      getClickObj(obj){
        if(obj.name.indexOf('SnowLink') > -1){
          //console.log(obj,'obj')
          return obj.name
        }else{
          //console.log(obj.parent)
          return this.getClickObj(obj.parent)
        }
      },
  }
  }
